    ...
    case 'varos':
      ...
      try {
        $dbh = new PDO('mysql:host=localhost;dbname=varosok', 'root', '',
                      array(PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION));
        $dbh->query('SET NAMES utf8 COLLATE utf8_hungarian_ci');
        $stmt = $dbh->prepare("select id, nev from varos where megyeid = :id");
    ...
    case 'info':
      ...
      try {
        $dbh = new PDO('mysql:host=localhost;dbname=varosok', 'root', '',
                      array(PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION));
        $dbh->query('SET NAMES utf8 COLLATE utf8_hungarian_ci');
        $stmt = $dbh->prepare("select ev, no, osszesen from lelekszam where varosid = :id");
        $stmt->execute(Array(":id" => $_POST["id"]));
        if($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
              $eredmeny = array("ev" => $row['ev'], "no" => $row['no'], "osszesen" => $row['osszesen']);
