...
function mo_llkszamok() {
    ...
    if (varosid != 0) {
        $.post(
            ...
            function(data) {
                $("#lszselect").html('<option value="0">Válasszon ...</option>');
                var lista = data.lista;
                for(i=0; i<lista.length; i++)
                    $("#lszselect").append('<option value="'+lista[i].id+'">'+lista[i].ev+'</option>');
            },
            "json"                                                    
        );
    }
...
$(document).ready(function() {
   ...
   $("#vkselect").change(mo_llkszamok);
   $("#lszselect").change(lelek);
